...
class Parameters:
    ...
    def build_parameters(self, ff, mol, terms):
        ...
        if any(elem in terms for elem in ["lj", "repulsioncg", "repulsion", "repulsionrna"]):
        ...
        if "dihedrals_rna" in terms and len(mol.dihedrals):
            self.dihedral_params = self.make_dihedrals(mol, ff)
...
def calculate_ABS(sigma, epsilon):
    # Lorentz - Berthelot combination rule
    sigma_table = 0.5 * (sigma + sigma[:, None])
    eps_table = torch.sqrt(epsilon * epsilon[:, None])
